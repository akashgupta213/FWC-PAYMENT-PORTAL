...
/* ─────────────────────────────────────────────
   Animated Shield SVG  (replaces WiFi on admin side)
───────────────────────────────────────────── */
function WifiIcon() {
  return (
    <svg viewBox="0 0 90 60" width="80" height="54"
...
/* ─────────────────────────────────────────────
   Icon helpers  (identical to Register.jsx)
───────────────────────────────────────────── */
...
const IconKey = ({ color = '#3a3a5c' }) => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
    <circle cx="8" cy="12" r="5" stroke={color} strokeWidth="1.8" />
    <path d="M13 12h8M18 10v4" stroke={color} strokeWidth="1.8" strokeLinecap="round" />
  </svg>
);
...
const IconShieldSmall = ({ color = '#444' }) => (
...
/* ─────────────────────────────────────────────
   Global CSS — extends student register styles
───────────────────────────────────────────── */
const CSS = `
...
@keyframes cwifi-draw { to { opacity: 1; stroke-dashoffset: 0; } }
  ...
  /* Admin accent — gold/amber tint replacing blue */
  .cadmin-badge {
    display: inline-flex;
    align-items: center;
    gap: 5px;
  ...
  /* Access code field gets gold focus */
  .cfield-access:focus { border-color:#d4a017 !important; box-shadow:0 0 0 3px rgba(212,160,23,0.18) !important; }

  .csubmit-admin { transition:transform 0.15s ease, box-shadow 0.15s ease, opacity 0.15s; }
  .csubmit-admin:hover:not(:disabled) { transform:translateY(-2px); box-shadow:0 10px 28px rgba(59,59,255,0.45); }
  .csubmit-admin:active:not(:disabled) { transform:translateY(0); }
  ...
  .cadmin-divider-line {
    flex: 1;
    height: 1px;
    background: linear-gradient(90deg, transparent, #252540, transparent);
  }
  .cadmin-divider-text {
    font-size: 10px;
    font-weight: 700;
...
export default function AdminRegister() {
  ...
  const [form, setForm] = useState({
    name: '', adminId: '', email: '', contact: '',
    password: '', confirm: '', accessCode: '',
  });
  ...
  /* inject CSS once */
  useEffect(() => {
    const id = 'comet-admin-register-styles';
  ...
  /* load logo */
  useEffect(() => {
    api.get('/assets').then(res => setAssets(res.data)).catch(() => {});
  }, []);
  ...
  const submit = async e => {
    ...
    if (!form.accessCode.trim())           return toast.error('Admin Access Code is required');
    if (!form.adminId.startsWith('COMET')) return toast.error('Admin ID must start with "COMET"');

    setLoading(true);
    try {
      await api.post('/auth/admin-register', {
        name:       form.name,
        adminId:    form.adminId,   // stored as cometId in DB with role:'admin'
        email:      form.email,
        contact:    form.contact,
        password:   form.password,
        accessCode: form.accessCode,
      });
      toast.success('Admin account created! Login with your Admin ID.');
  ...
  /* ── canvas particle network (identical logic to Register.jsx) ── */
  useEffect(() => {
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', minHeight: 60 }}>
              {assets.logo ? (
                ...
                <div style={{ color: '#2d55a0', fontWeight: 700 }}>Loading logo...</div>
              )}
            </div>
          </div>

          {/* Tagline */}
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingBottom: 300 }}>
            <div style={{ overflow: 'hidden', lineHeight: 1 }}>
              <span className="ctag-word ctag-word-1" style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 900, fontSize: 'clamp(52px,5.2vw,78px)', color: '#2d55a0', letterSpacing: '-0.5px', lineHeight: 1.05 }}>
                Future.
              </span>
            </div>
            <div style={{ overflow: 'hidden', lineHeight: 1, display: 'flex', alignItems: 'flex-end', gap: 8 }}>
              <span className="ctag-word ctag-word-2" style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 900, fontSize: 'clamp(52px,5.2vw,78px)', color: '#2d55a0', letterSpacing: '-0.5px', lineHeight: 1.05 }}>
                Wireless.
              </span>
              <span className="cshield-wrap" style={{ marginBottom: 'clamp(6px,0.8vw,12px)' }}>
                <WifiIcon />
              </span>
            </div>
            <div style={{ overflow: 'hidden', lineHeight: 1 }}>
              <span className="ctag-word ctag-word-3" style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 900, fontSize: 'clamp(52px,5.2vw,78px)', color: '#2d55a0', letterSpacing: '-0.5px', lineHeight: 1.05 }}>
                Communication.
              </span>
            </div>
        ...
        <div style={{ width: '100%', maxWidth: 440, paddingTop: 24, paddingBottom: 24 }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 30 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 14 }}>
              <span className="cadmin-badge">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none">
                  <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" stroke="#d4a017" strokeWidth="2.5" strokeLinejoin="round" />
                </svg>
                Admin Portal
              </span>
            </div>

            <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 58, height: 58, borderRadius: '50%', background: 'rgba(212,160,23,0.08)', border: '1.5px solid rgba(212,160,23,0.25)', marginBottom: 18 }}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
                <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" stroke="#d4a017" strokeWidth="2" strokeLinejoin="round" />
                <path d="M9 12l2 2 4-4" stroke="#d4a017" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            ...
            <p style={{ fontSize: 13, color: '#555', letterSpacing: 0.2 }}>
              IIITB COMET Foundation — Restricted Access
            </p>
          </div>
            ...
            <div className="cfield-grid">
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input name="name" value={form.name} onChange={handle} required autoComplete="name" placeholder="Admin Name" className="cfield" style={{ padding: '13px 14px 13px 42px' }} />
                </div>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#aaa', marginBottom: 8, letterSpacing: 0.2 }}>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input name="adminId" value={form.adminId} onChange={handle} required placeholder="COMETID" className="cfield" style={{ padding: '13px 14px 13px 42px' }} />
                </div>
              </div>
            </div>
...
<div>
  ...
  <div style={{ position: 'relative' }}>
    ...
    <input
      ...
      required
      autoComplete="email"
      placeholder="admin@iiitb.ac.in"
      className="cfield"
      pattern="[a-zA-Z0-9._%+\-]+@iiitb\.ac\.in"
      title="Only @iiitb.ac.in email addresses are allowed"
    ...
    />
            ...
            {/* Divider */}
            <div className="cadmin-divider">
              <div className="cadmin-divider-line" />
              <span className="cadmin-divider-text">Admin Verification</span>
              <div className="cadmin-divider-line" />
            </div>

            {/* Access Code — gold accented */}
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#d4a017', marginBottom: 8, letterSpacing: 0.2 }}>
                Admin Access Code <span style={{ color: '#ff4d4d' }}>*</span>
              </label>
              <div style={{ position: 'relative' }}>
                <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', display: 'flex' }}>
                  <IconKey color="#d4a017" />
                </span>
                <input
                  name="accessCode"
                  ...
                  className="cfield cfield-access"
                  style={{
                ...
                <button type="button" className="ceye" onClick={() => setShowCode(v => !v)}
                  ...
                  <IconEye open={showCode} color="#d4a017" />
                </button>
              </div>
              <p style={{ fontSize: 11, color: '#3a3a5c', marginTop: 6, letterSpacing: 0.2 }}>
                This code is provided by the system super-admin only.
              </p>
            </div>
          ...
          <p style={{ textAlign: 'center', fontSize: 13, color: '#444', marginTop: 22 }}>
            Already registered?{' '}
            <Link to="/admin-login" className="creg-link-blue">
              Admin Login
            </Link>
          </p>
          ...
          <div style={{ marginTop: 40, textAlign: 'center' }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#2a2a40' }}>
              <IconShieldSmall color="#2a2a40" />
              Restricted. Verified. Trusted.
            </div>
          </div>
        </div>
      </div>
